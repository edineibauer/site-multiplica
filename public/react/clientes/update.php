...
if (!empty($dados['id'])) {
    ...
    if (empty($cliente['login']) && !empty($nome) && (!empty($cliente[$email]) || !empty($cliente[$tel]))) {

        //Atualizou Cliente, não Existia Usuário, Requisitos Atendidos, Cria Usuário, Envia Email
        $senhaUser = (!empty($cliente['cpf']) ? substr($cliente['cpf'], 0, 4) : (!empty($cliente[$email]) ? explode("@", $cliente[$email])[0] : substr($cliente[$tel], 0, 4)));
        $user = [
            "nome" => $nome,
